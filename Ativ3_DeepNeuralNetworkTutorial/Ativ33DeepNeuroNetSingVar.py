'''
Rede Neural Profunda - Single Input
...
def build_and_compile_model(normalizer):
    """Ainda um modelo do tipo sequencial, esse terá duas camadas internas (ou ocultas) com
    função de ativação ReLU, e a camada de saída continua a mesma.
    No mais, o modelo continua funcionando como os anteriores, que eram apenas regressões lineares
    e buscavam otimizar (minimizar) a perda/loss.
    """
...
normalizer_horsepower = layers.Normalization(input_shape=[1,], axis=None)
normalizer_horsepower.adapt(horsepower)

log( '\nNormalization Means\n', normalizer_horsepower.mean.numpy(), sep='' )


""" Rede Neural Profunda com Uma Variável """

# Configura e compila o modelo usando a função definida na seção PROCEDIMENTOS.
model_dnn_horsepower = build_and_compile_model(normalizer_horsepower)

# Modelo construído. Sequential.summary() manda direto pra stdout.
if DEBUG:
    model_dnn_horsepower.summary()

log('\nTreinando modelo por 100 épocas...')
# Treina modelo com Model.fit() por 100 épocas.
model_fit_history = model_dnn_horsepower.fit(
...
test_results = model_dnn_horsepower.evaluate(
    test_features['Horsepower'],
    test_labels,
    verbose=0)


'''If you plot the predictions as a function of 'Horsepower', you should notice
 how this model takes advantage of the nonlinearity provided by the hidden layers:'''
if DEBUG:
    x = tf.linspace(0.0, 250, 251)
    y = model_dnn_horsepower.predict(x)
    plot_horsepower(x, y)
